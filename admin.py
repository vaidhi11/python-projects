...
def AdminLoginWindow():
    ...
    Button(AdminLoginPage, text='LOGIN', font=('Arial', 20, 'bold'), bg='#00ff99', fg='#f0f0f0',
           command=AdminLogin).place(relx=0.46, rely=0.55)
...
def AddBus():
    ...
    BusList = ttk.Treeview(AddBusForm.Right, columns=("bus_id", "bus_name"), selectmode='extended', height=500,
...
def RemoveBus():
    ...
    Label(RemoveBusForm, text='Remove a Bus from Database', font=('Arial Black', 10, 'bold'), bg='#f0f0f0').place(x=210,
                                                                                                                  y=88)
    ...
    BusList = ttk.Treeview(RemoveBusForm.Right, columns=("bus_id", "bus_name"), selectmode='extended', height=500,
                           yscrollcommand=scrollbary.set, xscrollcommand=scrollbarx.set)
    scrollbary.config(command=BusList.yview)
    scrollbary.pack(side=RIGHT, fill=Y)
    scrollbarx.config(command=BusList.xview)
...
def AdminHome():
    ...
    Button(AdminHomePage, text='Add a Bus to Database', font=('Arial Black', 10), width=50, height=30, bg='#00ff99',
           fg='#f0f0f0', bd=6, command=AddBus).place(relx=0.1, rely=0.2)
    Button(AdminHomePage, text='Remove a Bus from Database', font=('Arial Black', 10), width=50, height=30,
           bg='#00ff99', fg='#f0f0f0', bd=6, command=RemoveBus).place(relx=0.6, rely=0.2)
