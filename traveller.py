...
def TravellerLoginWindow():
    ...
    Button(TravellerLoginPage, text='LOGIN', padx=100, pady=5, font=('arial', 20, 'bold'), bg='#00ff99', fg='#f0f0f0',
           command=TravellerLogin).place(relx=0.39, y=500)
...
def TravellerHome(username):
    ...
    Label(TravellerHomePage, text=str(username) + '\'s Home', font=('impact', 30, 'bold'), bg='#00ff99').place(
        relx=0.42, y=15)

    Button(TravellerHomePage, text='Book a Ticket', font=('Arial Black', 10), width=50, height=30, bg='#00ff99',
           fg='#f0f0f0', bd=6,
           command=NewBooking).place(relx=0.03, rely=0.2)

    Button(TravellerHomePage, text='Check Ticket Status', font=('Arial Black', 10), width=50, height=30, bg='#00ff99',
           fg='#f0f0f0', bd=6,
           command=CheckStatus).place(relx=0.35, rely=0.2)

    Button(TravellerHomePage, text='Update Your Details', font=('Arial Black', 10), width=50, height=30, bg='#00ff99',
           fg='#f0f0f0', bd=6,
...
def BookTicket():
    ...
    try:
        cursor.execute(
            '''insert into booking values('{}', '{}', '{}', '{}', '{}', '{}', {}, {}, {});'''.format(str(tkt_no),
                                                                                                     username,
                                                                                                     dep_from.get(),
                                                                                                     dep_to.get(),
                                                                                                     dep_date.get(),
...
def CheckStatus():
    ...
    Button(StatusPage, text='CHECK STATUS', font=('Arial Black', 10), bg='#00ff99', fg='#f0f0f0', command=GetStatus,
           width=15).place(relx=0.4, rely=0.35)
    Button(StatusPage, text='EXIT', font=('Arial Black', 10), bg='#00ff99', fg='#f0f0f0', command=ExitStatus,
           width=15).place(relx=0.4, rely=0.39)
    ...
    TicketDetails = ttk.Treeview(StatusPage.Right, columns=('tkt_no', 'username', 'fname', 'lname', 'dep_from',
                                                            'dep_to', 'dep_date', 'meal', 'seat_no', 'bus_id', 'price',
                                                            'status'))
...
def UpdateDetails():
    ...
    Label(UpdateDetailsPage, text='Update User Details(any 1 field at a time)', font=('impact', 30, 'bold'),
          bg='#00ff99').place(relx=0.26, y=15)
    ...
    Button(UpdateDetailsPage, text='UPDATE', command=UpdateUser, font=('Arial', 10, 'bold'), width=20, bg='#00ff99',
           fg='#f0f0f0').place(relx=0.42, y=440)
    Button(UpdateDetailsPage, text='EXIT', command=ExitUpdateUser, font=('Arial', 10, 'bold'), width=20, bg='#00ff99',
           fg='#f0f0f0').place(relx=0.42, y=490)
...
def UpdateUser():
    ...
    try:
        if len(password.get()):
            cursor.execute(
                '''update customer set password='{}' where username='{}';'''.format(password.get(), username))
        elif len(email.get()):
...
def RegisterUser():
    ...
    Button(NewUserPage, text='REGISTER', command=AddUser, font=('Arial', 10, 'bold'), width=20, bg='#00ff99',
           fg='#f0f0f0').place(relx=0.42, y=440)
    Button(NewUserPage, text='EXIT', command=ExitNewUser, font=('Arial', 10, 'bold'), width=20, bg='#00ff99',
           fg='#f0f0f0').place(relx=0.42, y=490)
...
def AddUser():
    ...
    try:
        cursor.execute('''insert into customer values('{}', '{}', '{}', {}, '{}', '{}', '{}');'''.format(username.get(),
                                                                                                         password.get(),
                                                                                                         email.get(),
                                                                                                         age.get(),
                                                                                                         fname.get(),
